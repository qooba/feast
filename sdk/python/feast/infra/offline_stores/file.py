...
class PandasEvaluationEngine(EvaluationEngine):
    ...
    def _sort_offline_job_df(self, source_df, ts_columns):
        return source_df.sort_values(by=ts_columns)

    def _run_timestamp_mapping(
        self, df_to_join, event_timestamp_column: str, created_timestamp_column: str
    ):
    ...
    def _run_field_mapping(self, table, feature_view: FeatureView):
        ...
        if feature_view.projection.join_key_map:
            table = _run_field_mapping(table, feature_view.projection.join_key_map)
    ...
    def _rename_columns(self, df_to_join, columns_map):
        return df_to_join.rename(columns=columns_map)
    ...
    def _select_columns(self, df_to_join, right_entity_key_columns, feature_names):
        return df_to_join[right_entity_key_columns + feature_names]
    ...
    def evaluate_historical_retrieval(
        ...
        for feature_view, features in feature_views_to_features.items():
            ...
            entity_df_with_features = self._run_merge_asof(
                entity_df_with_features,
                df_to_join,
                entity_df_event_timestamp_col,
                event_timestamp_column,
    ...
    def evaluate_offline_job(
        ...
        source_df = self._read_data_offline(data_source)
        source_df = self._run_timestamp_mapping(
        ...
        source_df = self._sort_offline_job_df(source_df, ts_columns)
...
class DaskEvaluationEngine(PandasEvaluationEngine):
    ...
    def _sort_offline_job_df(self, source_df, ts_columns):
        return source_df.sort_values(by=ts_columns[0])
    ...
    def _run_field_mapping(self, table, feature_view: FeatureView):
        # Rename columns by the field mapping dictionary if it exists
        if feature_view.batch_source.field_mapping is not None:
            _run_dask_field_mapping(table, feature_view.batch_source.field_mapping)
        # Rename entity columns by the join_key_map dictionary if it exists
        if feature_view.projection.join_key_map:
            _run_dask_field_mapping(table, feature_view.projection.join_key_map)
    ...
    def _run_merge_asof(
        ...
        entity_df_with_features = entity_df_with_features.persist()
...
class FileOfflineStore(OfflineStore):
    ...
    def get_historical_features(
        ...
        def evaluate_historical_retrieval(use_dask=False):
            ...
            impl = DaskEvaluationEngine() if use_dask else PandasEvaluationEngine()
            return impl.evaluate_historical_retrieval(
                feature_views,
                entity_df,
                registry,
                project,
    ...
    def pull_latest_from_table_or_query(
        ...
        def evaluate_offline_job(use_dask=False):
            ...
            impl = DaskEvaluationEngine() if use_dask else PandasEvaluationEngine()
            return impl.evaluate_offline_job(
                data_source,
                join_key_columns,
